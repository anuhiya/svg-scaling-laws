...
print(f"Original: {len(raw_svgs)}, too short: {n_short}, invalid XML: {n_bad_xml}, after dedup: {len(deduped)}")
...
print(f"Too long: {n_too_long}, final dataset: {len(tokenized)} SVGs, {total_tokens} tokens")

if total_tokens < TARGET_TRAIN_TOKENS:
    print(f"Warning: only {total_tokens/1e6:.1f}M tokens, below 100M target")
else:
    print(f"Token target met: {total_tokens/1e6:.1f}M tokens")
...
for name, idxs in splits.items():
    ...
    print(f"{name}: {len(idxs)} SVGs, {n_toks} tokens")
...
try:
    ...
    for label, (svg, ids) in examples.items():
        ...
        try:
            ...
            print(f"Rendered {label} ({len(ids)} tokens)")
